...
- **Artifact position:** `W02D02 power-on-to-main / startup-sequence artifact — ARTIFACT_PASS`
- **Competency position:** `W01-C-FOUND — COMPETENCY_PASS`
- **Last artifact PASS:** `W02D02 power-on-to-main artifact`, evidence at `learning/week-02/day-02/POWER_ON_TO_MAIN_W02D02.md` and `learning/week-02/day-02/SUBMIT_W02_D02.md`
...
- **Branch:** `main`
- **Artifact closure:** W02D02 END DAY closure transaction; containing commit recorded as `SELF — containing commit` until repository history resolves it
...
- **Build command:** `powershell -ExecutionPolicy Bypass -File .\build.ps1 -Clean` from `firmware/stm32/w02d02-startup-lab`.
- **Latest build result:** `PASS / exit 0`; ELF/map/list generated. Linker emitted non-blocking `nosys` warnings for `_close`, `_lseek`, `_read` and `_write`.
- **Test command:** `N/A — required validation was clean build plus source/map/list inspection`.
- **Latest test result:** `N/A`; fresh map reproduced all recorded startup symbols and `.data`/`.bss` ranges.
...
- **Schedule variance:** `W02D02 closed on its roadmap date`; Actual Focused Time was `~2h56m — learner estimate`. Planned Focused Time is an `UNRECOVERABLE PROCESS VARIANCE` because the Day Contract did not preserve a trusted numeric value; no schedule debt is inferred. The transient `6h` availability is not substituted for Planned Focused Time.
...
- **Health/load:** No new health score was supplied for W02D02. Actual Focused Time: `~2h56m — learner estimate`. No schedule debt or Recovery condition is inferred from completing the authoritative stop condition in less than the transient `6h` availability.
