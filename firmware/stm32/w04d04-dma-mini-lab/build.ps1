...
try {
    ...
    & $gcc @commonFlags '-std=gnu11' '-c' 'dma_m2m.c' '-o' 'build/dma_m2m.o'
    if ($LASTEXITCODE -ne 0) { throw 'dma_m2m.c compilation failed.' }
    ...
    $linkFlags = @(
        ...
        '--specs=nosys.specs',
        '--specs=nano.specs',
        '-Wl,-Map=build/w04d04-dma-mini-lab.map',
        '-Wl,--gc-sections',
        '-static',
        ...
        '-Wl,--end-group',
        '-o',
        'build/w04d04-dma-mini-lab.elf'
    )
    & $gcc @linkFlags
    if ($LASTEXITCODE -ne 0) { throw 'Link failed.' }

    & $size 'build/w04d04-dma-mini-lab.elf'
    if ($LASTEXITCODE -ne 0) { throw 'Size inspection failed.' }

    & $objdump '-h' '-S' 'build/w04d04-dma-mini-lab.elf' | Set-Content -LiteralPath 'build/w04d04-dma-mini-lab.list' -Encoding ascii
