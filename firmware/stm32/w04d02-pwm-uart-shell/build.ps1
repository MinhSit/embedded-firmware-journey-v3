...
try {
    ...
    $linkFlags = @(
        ...
        '--specs=nosys.specs',
        '--specs=nano.specs',
        '-Wl,-Map=build/w04d02-pwm-uart-shell.map',
        '-Wl,--gc-sections',
        '-static',
        ...
        '-Wl,--end-group',
        '-o',
        'build/w04d02-pwm-uart-shell.elf'
    )
    & $gcc @linkFlags
    if ($LASTEXITCODE -ne 0) { throw 'Link failed.' }

    & $size 'build/w04d02-pwm-uart-shell.elf'
    if ($LASTEXITCODE -ne 0) { throw 'Size inspection failed.' }

    & $objdump '-h' '-S' 'build/w04d02-pwm-uart-shell.elf' | Set-Content -LiteralPath 'build/w04d02-pwm-uart-shell.list' -Encoding ascii
