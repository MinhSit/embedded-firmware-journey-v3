...
try {
    ...
    & $gcc @commonFlags '-std=gnu11' '-c' 'fault_diag.c' '-o' 'build/fault_diag.o'
    if ($LASTEXITCODE -ne 0) { throw 'fault_diag.c compilation failed.' }
    ...
    $linkFlags = @(
        ...
        '--specs=nosys.specs',
        '--specs=nano.specs',
        '-Wl,-Map=build/w04d05-hardfault-triage.map',
        '-Wl,--gc-sections',
        '-static',
        ...
        '-Wl,--end-group',
        '-o',
        'build/w04d05-hardfault-triage.elf'
    )
    & $gcc @linkFlags
    if ($LASTEXITCODE -ne 0) { throw 'Link failed.' }

    & $size 'build/w04d05-hardfault-triage.elf'
    if ($LASTEXITCODE -ne 0) { throw 'Size inspection failed.' }

    & $objdump '-h' '-S' 'build/w04d05-hardfault-triage.elf' | Set-Content -LiteralPath 'build/w04d05-hardfault-triage.list' -Encoding ascii
