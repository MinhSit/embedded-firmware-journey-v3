...
try {
    ...
    & $gcc @commonFlags '-std=gnu11' '-c' 'rx_ring_buffer.c' '-o' 'build/rx_ring_buffer.o'
    if ($LASTEXITCODE -ne 0) { throw 'rx_ring_buffer.c compilation failed.' }

    & $gcc @commonFlags '-std=gnu11' '-c' 'uart_parser.c' '-o' 'build/uart_parser.o'
    if ($LASTEXITCODE -ne 0) { throw 'uart_parser.c compilation failed.' }
    ...
    $linkFlags = @(
        ...
        '--specs=nosys.specs',
        '--specs=nano.specs',
        '-Wl,-Map=build/w03d04-uart-parser-lab.map',
        '-Wl,--gc-sections',
        '-static',
        ...
        '-Wl,--end-group',
        '-o',
        'build/w03d04-uart-parser-lab.elf'
    )
    & $gcc @linkFlags
    if ($LASTEXITCODE -ne 0) { throw 'Link failed.' }

    & $size 'build/w03d04-uart-parser-lab.elf'
    if ($LASTEXITCODE -ne 0) { throw 'Size inspection failed.' }

    & $objdump '-h' '-S' 'build/w03d04-uart-parser-lab.elf' | Set-Content -LiteralPath 'build/w03d04-uart-parser-lab.list' -Encoding ascii
