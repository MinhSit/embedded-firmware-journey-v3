...
try {
    ...
    & $gcc @commonFlags '-std=gnu11' '-c' 'timer_timebase.c' '-o' 'build/timer_timebase.o'
    if ($LASTEXITCODE -ne 0) { throw 'timer_timebase.c compilation failed.' }
    ...
    $linkFlags = @(
        ...
        '--specs=nosys.specs',
        '--specs=nano.specs',
        '-Wl,-Map=build/w04d01-timer-timebase.map',
        '-Wl,--gc-sections',
        '-static',
        'build/startup_stm32f446retx.o',
        'build/system_stm32f4xx.o',
        'build/main.o',
        'build/timer_timebase.o',
        '-Wl,--start-group',
        '-lc',
        '-lm',
        '-Wl,--end-group',
        '-o',
        'build/w04d01-timer-timebase.elf'
    )
    & $gcc @linkFlags
    if ($LASTEXITCODE -ne 0) { throw 'Link failed.' }

    & $size 'build/w04d01-timer-timebase.elf'
    if ($LASTEXITCODE -ne 0) { throw 'Size inspection failed.' }

    & $objdump '-h' '-S' 'build/w04d01-timer-timebase.elf' | Set-Content -LiteralPath 'build/w04d01-timer-timebase.list' -Encoding ascii
