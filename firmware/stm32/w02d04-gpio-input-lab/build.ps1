...
try {
    ...
    $linkFlags = @(
        ...
        '--specs=nosys.specs',
        '--specs=nano.specs',
        '-Wl,-Map=build/w02d04-gpio-input-lab.map',
        '-Wl,--gc-sections',
        '-static',
    ...
    & $size 'build/w02d04-gpio-input-lab.elf'
    if ($LASTEXITCODE -ne 0) { throw 'Size inspection failed.' }

    & $objdump '-h' '-S' 'build/w02d04-gpio-input-lab.elf' | Set-Content -LiteralPath 'build/w02d04-gpio-input-lab.list' -Encoding ascii
