...
try {
    ...
    & $gcc @commonFlags '-std=gnu11' '-c' 'debounce.c' '-o' 'build/debounce.o'
    if ($LASTEXITCODE -ne 0) { throw 'debounce.c compilation failed.' }
    ...
    $linkFlags = @(
        ...
        '--specs=nosys.specs',
        '--specs=nano.specs',
        '-Wl,-Map=build/w02d06-debounce-lab.map',
        '-Wl,--gc-sections',
        '-static',
        ...
        '-Wl,--end-group',
        '-o',
        'build/w02d06-debounce-lab.elf'
    )
    & $gcc @linkFlags
    if ($LASTEXITCODE -ne 0) { throw 'Link failed.' }

    & $size 'build/w02d06-debounce-lab.elf'
    if ($LASTEXITCODE -ne 0) { throw 'Size inspection failed.' }

    & $objdump '-h' '-S' 'build/w02d06-debounce-lab.elf' | Set-Content -LiteralPath 'build/w02d06-debounce-lab.list' -Encoding ascii
