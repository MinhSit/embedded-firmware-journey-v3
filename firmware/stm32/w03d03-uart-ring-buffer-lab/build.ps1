...
try {
    ...
    $linkFlags = @(
        ...
        '--specs=nosys.specs',
        '--specs=nano.specs',
        '-Wl,-Map=build/w03d03-uart-ring-buffer-lab.map',
        '-Wl,--gc-sections',
        '-static',
        ...
        '-Wl,--end-group',
        '-o',
        'build/w03d03-uart-ring-buffer-lab.elf'
    )
    & $gcc @linkFlags
    if ($LASTEXITCODE -ne 0) { throw 'Link failed.' }

    & $size 'build/w03d03-uart-ring-buffer-lab.elf'
    if ($LASTEXITCODE -ne 0) { throw 'Size inspection failed.' }

    & $objdump '-h' '-S' 'build/w03d03-uart-ring-buffer-lab.elf' | Set-Content -LiteralPath 'build/w03d03-uart-ring-buffer-lab.list' -Encoding ascii
